...
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, TwoWire *twi,
                                   ...
                                   uint32_t clkAfter)
    : Adafruit_GFX(w, h), spi(NULL), wire(twi ? twi : &Wire), buffer_display(NULL),
...
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, int8_t mosi_pin,
    ...
    : Adafruit_GFX(w, h), spi(NULL), wire(NULL), buffer_display(NULL),
...
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, SPIClass *spi,
                                   ...
                                   uint32_t bitrate)
    : Adafruit_GFX(w, h), spi(spi ? spi : &SPI), wire(NULL), buffer_display(NULL),
...
Adafruit_SSD1306::Adafruit_SSD1306(int8_t mosi_pin, int8_t sclk_pin,
    ...
    : Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(NULL), wire(NULL),
      buffer_display(NULL), mosiPin(mosi_pin), clkPin(sclk_pin), dcPin(dc_pin),
...
Adafruit_SSD1306::Adafruit_SSD1306(int8_t dc_pin, int8_t rst_pin, int8_t cs_pin)
    : Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(&SPI), wire(NULL),
      buffer_display(NULL), mosiPin(-1), clkPin(-1), dcPin(dc_pin), csPin(cs_pin),
      rstPin(rst_pin) {
#ifdef SPI_HAS_TRANSACTION
...
Adafruit_SSD1306::Adafruit_SSD1306(int8_t rst_pin)
    : Adafruit_GFX(SSD1306_LCDWIDTH, SSD1306_LCDHEIGHT), spi(NULL), wire(&Wire),
      buffer_display(NULL), mosiPin(-1), clkPin(-1), dcPin(-1), csPin(-1),
      rstPin(rst_pin) {}
...
bool Adafruit_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset,
  ...
  if ((!buffer_display) && !(buffer_display = (uint8_t *)malloc(WIDTH * ((HEIGHT + 7) / 8)))){
    
    for (int i=0; i<10; i++){
      unsigned long value = *(buffer_display+i);
      Serial.println(value); //this doesn't work
    }
    Serial.println();
     return false;
    //return true;
...
void Adafruit_SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    ...
    switch (color) {
    case SSD1306_WHITE:
      buffer_display[x + (y / 8) * WIDTH] |= (1 << (y & 7));
      break;
    case SSD1306_BLACK:
      buffer_display[x + (y / 8) * WIDTH] &= ~(1 << (y & 7));
      break;
...
void Adafruit_SSD1306::clearDisplay(void) {
  memset(buffer_display, 0, WIDTH * ((HEIGHT + 7) / 8));
}
...
void Adafruit_SSD1306::drawFastHLineInternal(int16_t x, int16_t y, int16_t w,
  ...
  if ((y >= 0) && (y < HEIGHT)) { // Y coord in bounds?
    ...
    if (w > 0) { // Proceed only if width is positive
      uint8_t *pBuf = &buffer_display[(y / 8) * WIDTH + x], mask = 1 << (y & 7);
      switch (color) {
      case SSD1306_WHITE:
        while (w--) {
          *pBuf++ |= mask;
...
void Adafruit_SSD1306::drawFastVLineInternal(int16_t x, int16_t __y,
  ...
  if ((x >= 0) && (x < WIDTH)) { // X coord in bounds?
    ...
    if (__h > 0) { // Proceed only if height is now positive
      ...
      uint8_t y = __y, h = __h;
      uint8_t *pBuf = &buffer_display[(y / 8) * WIDTH + x];
...
bool Adafruit_SSD1306::getPixel(int16_t x, int16_t y) {
  if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    ...
    return (buffer_display[x + (y / 8) * WIDTH] & (1 << (y & 7)));
  }
...
*/
uint8_t *Adafruit_SSD1306::getBuffer(void) { return buffer_display; }
