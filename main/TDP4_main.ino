...
bool scheduleUpDown(void *) {
  if (!party) { //oscillating
    Serial.println("not a party");
    flashLED(4, LEDup, false);
    LEDup = !LEDup;
  }
  else { //party mode
    flashLED(16, true, false);
    Serial.println("party");

    //random colour next time
    rBaseLevel = int(random(1, 255));
    gBaseLevel = int(random(1, 255));
    bBaseLevel = int(random(1, 255));
...
void loop() {
  ...
  //one of the MODEs already selected
  else {
    ...
    //mode for displaying battery
      ...
      if (fallingEdge(push1_old, push1_new) && fallingEdge(push2_old, push2_new)) {
        //PARTY MODE
        party = !party;
        if (party) {
          scheduleLEDs.cancel();
          scheduleLEDs.every(300, scheduleUpDown);
        } else {
          scheduleLEDs.cancel();
        }
      }
